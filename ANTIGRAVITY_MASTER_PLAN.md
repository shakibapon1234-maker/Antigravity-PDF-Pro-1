...
- **ব্যাকগ্রাউন্ড রিমোট চেক:** পিসি যখনই ইন্টারনেটে যুক্ত হবে, অ্যাপটি ব্যাকগ্রাউন্ডে লাইসেন্স স্ট্যাটাস চেক করবে। অ্যাপটি ৩০ দিন অনলাইন না হলেও একটা গ্রেস পিরিয়ড দেবে; তারপর পুনরায় অনলাইন ভেরিফিকেশন দরকার হবে।
...
### Issue #1 — CDN Dependency (fixed)
- **ফাইল:** index.html
- **সমস্যা:** pdf.js, pdf-lib, lucide CDN থেকে লোড হওয়ার কারণে আগে Internet dependency ছিল
- **স্ট্যাটাস:** এখন `ui/libs/` এ স্থানান্তরিত করা হয়েছে, অফলাইন ব্যবহার সহজ আছে
- **Fix:** Phase 0-A সম্পন্ন

### Issue #2 — electron-store ঠিক আছে
- **স্ট্যাটাস:** package.json এ `electron-store` আছে এবং `main.js` / `preload.js` দিয়ে স্টোর IPC সমর্থন হচ্ছে
- **Note:** এখন Recent Files, Theme, Settings ইত্যাদি electron-store ব্যবহার করতে পারে

### Issue #3 — Error Logging যোগ করা হয়েছে
- **সমস্যা:** আগে App ক্র্যাশ করলে কোনো log ছিল না
- **Fix:** এখন `electron-log` main ও server উভয় দিকেই লগ করছে
- **Note:** লগ ফাইল `userData/.logs/` এ থাকবে
...
## 🔵 PHASE 4 — নতুন Advanced Features (2026-06-08)

### 4-A. Thumbnail Sidebar
- **ফাইল:** `tools/thumbnail-sidebar.js`
- **বিবরণ:** Editor বাম পাশে সব পেজের ছোট thumbnail দেখায়। Click করলে সেই পেজে যাওয়া যায়। Lazy rendering — শুধু visible thumbnails render হয়।
- **Toolbar button:** "Thumbs" বোতাম (toggle)
- **স্ট্যাটাস:** ✅ সম্পন্ন (2026-06-08)

### 4-B. Find & Replace
- **ফাইল:** `tools/find-replace.js`
- **বিবরণ:** Ctrl+F দিয়ে অথবা toolbar "Find" বোতামে floating panel খুলবে। PDF এর text search করা যাবে, highlight দেখাবে, navigate করা যাবে। Replace option ও আছে।
- **Keyboard:** Ctrl+F toggle, Enter = next, Shift+Enter = prev, Esc = close
- **স্ট্যাটাস:** ✅ সম্পন্ন (2026-06-08)

### 4-C. PDF Comparison Tool
- **ফাইল:** `tools/pdf-compare.js`
- **বিবরণ:** দুটো PDF side-by-side দেখায়। Pixel-level diff canvas মাঝখানে দেখায় — লাল = পার্থক্য। Text stats (added/removed words) দেখায়।
- **Toolbar button:** "Compare" বোতাম
- **স্ট্যাটাস:** ✅ সম্পন্ন (2026-06-08)

### 4-D. Multi-Format Export
- **ফাইল:** `tools/multi-export.js`
- **বিবরণ:** একটা PDF থেকে একসাথে: PDF, PNG images (ZIP), Plain Text (.txt), HTML, JSON export করা যায়।
- **Toolbar button:** "Export" বোতাম
- **স্ট্যাটাস:** ✅ সম্পন্ন (2026-06-08)

### Phase 4 Integration Points
- `core/renderer.js` → PDF load হলে `window._currentPdfDoc`, `window._currentFileObj` সেট হয়, ThumbnailSidebar ও FindReplace কে hook করে
- `editor/init.js` → `window.goToPage()` global function যোগ করা হয়েছে thumbnail navigation এর জন্য
- `index.html` → Toolbar এ ৪টা নতুন button (Thumbs, Find, Export, Compare)
- `style.css` → phase4.css append করা হয়েছে
